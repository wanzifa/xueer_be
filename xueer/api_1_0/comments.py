...
def get_courses_id_comments(id):
    ...
    pagination = course.comment.order_by(Comments.timestamp.desc()).paginate(
    ...
    if pagination.has_prev:
        prev = url_for('api.get_courses_id_comments', page=page - 1, _external=True)
    next = ""
    if pagination.has_next:
        next = url_for('api.get_courses_id_comments', page=page + 1, _external=True)
    comments_count = len(Comments.query.filter_by(course_id=id).all())
    if comments_count % current_app.config["XUEER_COMMENTS_PER_PAGE"] == 0:
...
def get_tip_id_comments(id):
    ...
    prev = ""
    if pagination.has_prev:
        prev = url_for('api.get_tip_id_comments', page=page - 1, _external=True)
    next = ""
    if pagination.has_next:
        next = url_for('api.get_tip_id_comments', page=page + 1, _external=True)
    comments_count = len(Comments.query.filter_by(course_id=id).all())
